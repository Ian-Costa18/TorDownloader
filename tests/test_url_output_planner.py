...
from tor_downloader.utils.output_layout import (
...
from tor_downloader.utils.url_utils import (
...
def test_output_layout_helpers() -> None:
    ...
    assert (
        relative_path_from_url("http://host/base/one/two.txt", keep_filename=True)
        == "base/one/two.txt"
    )
    assert (
        relative_path_from_url("http://host/base/one/two.txt", keep_filename=False)
        == "base/one"
    )
...
def test_plan_download_jobs_mirror_mode() -> None:
    spec = LinksSpec(
        ...
        files=[
            "folder/file1.bin",
            "folder/dir2/",
            "http://single.onion/path/file2.dat",
        ],
    )
    ...
    absolute_job = next(
        job for job in jobs if job.source_entry.startswith("http://single.onion")
    )
